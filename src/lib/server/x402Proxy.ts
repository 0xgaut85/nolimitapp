...
export async function proxyX402Request(request: NextRequest, targetPath: string) {
  const serverUrl = getX402ServerUrl();
  ...
  const targetUrl = `${serverUrl}${normalizedPath}`;
