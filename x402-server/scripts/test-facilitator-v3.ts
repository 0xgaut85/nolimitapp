/**
 * Test facilitator directly - using the EXACT format from useFacilitator
 */
...
async function testFacilitator() {
  ...
  console.log('=== FACILITATOR TEST V3 (correct format) ===\n');
  ...
  // Create payment payload as OBJECT (not base64)
  // This is what useFacilitator sends
  ...
  // This is the exact format useFacilitator sends
  const requestBody = {
  ...
  try {
    ...
    try {
      ...
      console.log('Parsed:', JSON.stringify(verifyData, null, 2));
      ...
      if (verifyData.isValid) {
        ...
        try {
          const settleData = JSON.parse(settleText);
          console.log('Parsed:', JSON.stringify(settleData, null, 2));
          
          if (settleData.success) {
            console.log('\n✅ Settlement successful! Transaction:', settleData.transaction);
          } else {
            console.log('\n❌ Settlement failed:', settleData.errorReason);
          }
        } catch (e) {
          console.log('Could not parse settle response');
        }
      } else {
